...
install_dependencies() {
    ...
    mkdir -p "$apt_root"/{empty,home,lists/partial,archives/partial}
    : >"$apt_root/empty.conf"
...
tab="$(printf '\t')"
for required in base-files dpkg openssh-server ca-certificates; do
...
Mirror         : $mirror (signed metadata and SHA-256 verification required)
Root FS        : $filesystem
...
Execution mode : unattended preparation; reboot remains manual
EOF
}
