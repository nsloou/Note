...
cleanup_apt_cache() {
    ...
    if ! rm -rf -- "$apt_cache_dir"; then
        warn "准备阶段 APT 缓存清理失败：$apt_cache_dir"
        return 1
    fi
    apt_cache_dir=
}
...
EOF
    ...
    log "仅从 Debian $source_codename 官方仓库安装准备工具"
    write_dependency_sources https
    if ! clean_apt "${apt_options[@]}" update; then
        warn "官方 HTTPS APT 失败；尝试通过仍须验签的 Debian 官方 HTTP 源修复 CA 和密钥环"
...
disk_facts() {
    ...
    if [ -z "$disk_ptuuid" ]; then
        warn "无法读取目标磁盘分区表 UUID；将依靠容量及可用分区标识进行匹配"
    fi
    if [ -z "$disk_anchor_partuuid" ]; then
        warn "目标磁盘没有可用的分区 PARTUUID；必须通过容量和 UUID 唯一匹配"
    fi
}
...
load_package_indexes() {
    ...
    log "正在下载软件包索引，并按已签名 Release 的哈希链校验"
...
unpack_initrd() {
    ...
    mkdir -p "$initrd_dir"
    log "正在解包 Debian Installer 的 initrd"
    (
        cd "$initrd_dir"
...
install_verified_udebs() {
    ...
    log "正在加入经过校验的 Debian timeout 工具，为安装器探测设置超时"
...
copy_module_udeb_if_available() {
    ...
    if [ -n "$(package_fields "$package_index_udeb" "$package")" ]; then
        ...
        warn "$arch 未发布此安装器模块包，将跳过：$package"
    fi
}
...
add_hyperv_pci_module_if_needed() {
    ...
    log "正在加入本 Hyper-V/Azure 虚拟机所需且经过校验的 pci_hyperv 模块"
...
disable_source_kexec_bypass() {
    ...
    if [ -f "$KEXEC_DEFAULT" ] &&
        ...
        log "已临时禁用旧系统 kexec；--reset 可恢复其原有配置"
    fi
}
